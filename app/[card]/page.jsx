...
import { serviceProvider } from "../service/homeInteriorService";
const page = () => {
  ...
  async function Desc() {
    try {
      const response = await serviceProvider(card);
      console.log("inside try", card);
      console.log(response);
      setDesc(response.services);
    } catch (error) {
      console.log(error);
    }
  }
          ...
          {desc.map((service) => (
            <>
              ...
              <div className=" max-w-lg  bg-white border border-gray-200 rounded-lg shadow   mb-5 ml-2">
                  ...
                  <img
                    className="p-8 rounded-t-lg hover:p-0 cursor-pointer"
                    src={service.image}
                    alt="product image"
                  onClick={e=>router.push(`${service.provider_id}/${service.id}`)} key={service.id}
                />
